...
import { Plus, Play, Trash2, Search, Music, Folder, ArrowLeft, FolderPlus, MoveRight } from 'lucide-react';
...
const Library = ({ onSelectProject }) => {
    ...
    const displayedItems = allItems.filter(item => {
        // 1. Must match current Folder
        if (item.parentId !== currentFolderId && !(item.parentId === undefined && currentFolderId === null)) {
            return false;
        }

        // 2. Search filter (if search active, maybe show all matches? For now keep strict scope)
        if (searchTerm) {
    ...
    return (
        <div className="flex flex-col h-screen bg-slate-950 text-slate-100 p-6 relative">
            ...
            <div className="relative mb-6">
                ...
                <input
                    ...
                    className="w-full bg-slate-900 border border-slate-800 rounded-xl py-3 pl-10 pr-4 text-slate-200 focus:outline-none focus:border-violet-500 transition"
                />
            </div>
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm text-slate-500">
                                        {item.type === 'folder' ? 'Folder' : (item.artist || 'Unknown Artist')}
                                    </p>
                                </div>
                            </div>
